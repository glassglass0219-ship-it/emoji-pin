...
def main() -> int:
    ap = argparse.ArgumentParser(description="characters.json の名前変更/統合（v7）+ thumbnails 整理")
    ...
    # 1) rename-only ("A → B")
    renames = [
        ("トラファルガー・ロー", "トラファルガー・ロー（トラファルガー・Ｄ・ワーテル・ロー）"),
        ("ビール", "ビール6世"),
        ("ズニーシャ", "像主（ズニーシャ）"),
        ("針神", "針神様"),
        ("ドラウグル", "どらうぐる（MMA）"),
    ...
    for old, new in renames:
        try:
            cid = find_single_id_by_name(live_chars(), old)
        except KeyError as e:
            missing.append(f"RENAME {old} → {new}: {e}")
            continue
        ch = by_id.get(cid)
        if not isinstance(ch, dict):
            missing.append(f"RENAME {old} → {new}: id not found")
            continue
        if nfkc(ch.get("name") or "") != nfkc(new):
            ch["name"] = nfkc(new)
            renamed_count += 1

    # merge
    for left, right, final_name in merges:
        try:
            id1 = find_single_id_by_name(live_chars(), left)
            id2 = find_single_id_by_name(live_chars(), right)
        except KeyError as e:
            missing.append(f"MERGE {left} / {right} → {final_name}: {e}")
            continue
        ...
        if survivor is None or removed is None:
            missing.append(f"MERGE {left} / {right} → {final_name}: id not found")
            continue
        merge_two(survivor, removed, final_name)
    ...
    changed_total = renamed_count + merged_count
    print(f"変更（名前変更）件数: {renamed_count}")
    print(f"統合（マージ）件数: {merged_count}")
    print(f"合計（統合または名前変更）件数: {changed_total}")
