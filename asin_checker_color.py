...
def get_asin_list():
    ...
    print("--- ONE PIECE カラー版 ASIN抽出開始 (Amazon.co.jp) ---")
    print("※日本の検索結果を直接確認しています...")

    for vol in range(1, 4):
        query = f"ONE PIECE カラー版 {vol} ジャンプコミックスDIGITAL"
        ...
        try:
            ...
            for item in items:
                ...
                # カラー版のみ対象、巻数一致を厳格にチェック
                if "カラー版" in title and str(vol) in title:
                    print(f"第{vol}巻: {asin} | タイトル: {title[:50]}...")
                    found = True
                    break
