...
def main() -> int:
    ap = argparse.ArgumentParser(description="characters.json の統合（v9）+ thumbnails 整理")
    ...
    left_name = "バッキンガム・ステューシー"
    right_name = "ミス・バッキン"

    id1 = find_single_id_by_name(chars, left_name)
    id2 = find_single_id_by_name(chars, right_name)
    ...
    if survivor is None or removed is None:
        raise SystemExit("ID解決に失敗しました")

    merge_two(survivor, removed, left_name)
    thumb_action = handle_thumbnails_merge(thumb_dir, survivor_id, removed_id)
    ...
    print(f"統合: {left_name} / {right_name}")
    print(f"存続ID: {survivor_id}")
    print(f"削除ID: {removed_id}")
    print(f"最終名: {left_name}")
    print(f"画像: {thumb_action}")
    return 0
