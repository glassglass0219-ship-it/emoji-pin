...
def main() -> int:
    ap = argparse.ArgumentParser(description="characters.json の統合（v10）+ thumbnails 整理")
    ...
    print(f"統合（マージ）件数: {len(results)}")
    for r in results:
        print(f"  - {r['pair']}: {r['survivor_id']} を存続 / {r['removed_id']} を削除 → name='{r['final_name']}' (thumb={r['thumb']})")
